...
def do_sobelx(img):
    ...
    # Threshold x gradient
    sxxbinary = np.zeros_like(scaled_sobelxx)
    sxxbinary[(scaled_sobelxx >= 50)] = 1
    sxxbinary = sxxbinary * (mask / 255)
...
def get_boxes(image_base, image_out):
    ...
    for cntr in contours:
        ...
        if (w > 10 and h > 10):
            cv2.rectangle(image_out, (x-10, y), (x + w, y + h), (0, 0, 255), 2)
            cv2.rectangle(im_check, (x-10, y), (x + w, y + h), (0, 255, 255), -1)
            count += 1
...
if __name__ == '__main__':
    ...
    for im_targ, gt in zip(images, grounds):
        ...
        axs[0].imshow(img_out, cmap='gray')
        axs[1].imshow(im_check, cmap='gray')
        plt.show()
